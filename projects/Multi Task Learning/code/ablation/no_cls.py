...
class MMoE(nn.Module):
    def __init__(self, num_expert=2, num_gate=2):
        ...
        self.expert_box = ExpertBox(BasicBlock, layers=[2, 2, 2, 2])
        self.tower_box = TowerBox(BasicBlock, layers=[2, 2, 2, 2])
    ...
    def forward(self, input):
        ...
        size_box = torch.cat((expert_box, expert_Unet_tran), 1).size()
        size_U = torch.cat((expert_Unet[-1], expert_box_tran), 1).size()
        ...
        expert_out_box = torch.sum(gate_box * torch.cat((expert_box, expert_Unet_tran), 1), 1)
        expert_out_Unet = torch.sum(gate_Unet * torch.cat((expert_Unet[-1], expert_box_tran), 1), 1)
...
if __name__ == '__main__':
    ...
    expert_box = list(map(id, mmoe.expert_box.parameters()))
    tower_box = list(map(id, mmoe.tower_box.parameters()))
    base_params = filter(lambda p: id(p) not in expert_box + tower_box, 
                        mmoe.parameters())
    ...
    for epoch in range(epochs):
        ...
        for i, (images, labels) in enumerate(progress_bar):
            ...
            optimizer.zero_grad()
            output_seg, output_box = mmoe(images)
            ...
            loss = 0.5 * loss_seg + 0.5 * loss_box
            loss.backward()
            optimizer.step()
        ...
        train_bbox_iou_list.append(bbox_iou_avg / (i + 1))
        ...
        with torch.no_grad():
            ...
            test_bbox_iou_list.append(bbox_iou_avg / (i + 1))
        
    print('Training done.')
    torch.save(mmoe.state_dict(), 'saved_mmoe_no_cls.pt')
    ...
    plt.plot(np.arange(101), train_iou, color='orange', label='training IoU of segmentation', linestyle='-')
    ...
    plt.savefig('ablation_no_cls', dpi=500)
    plt.show()
