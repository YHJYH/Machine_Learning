...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=5e-5, eps=1e-5)
    ...
    for epoch in range(epochs):
        ...
        for i, (images, labels) in enumerate(progress_bar):
            ...
            optimizer.zero_grad()
            output_cla = model(images)
            loss = criterion(output_cla, binarys.type(torch.cuda.FloatTensor))
            loss.backward()
            optimizer.step()
    ...
    plt.plot(np.arange(len(train_loss_list)), train_loss_list, color='b', label='training loss of classification', linestyle="-")
    plt.plot(np.arange(len(test_loss_list)), test_loss_list, color='b', label='testing loss of classification', linestyle="--")
    plt.legend()
    plt.grid()
    plt.ylabel('Loss')
    plt.xlabel('Epochs')
    plt.title("Loss vs Epochs(100) of classification")
    plt.savefig('loss_cls.png', dpi=500)
    plt.show()